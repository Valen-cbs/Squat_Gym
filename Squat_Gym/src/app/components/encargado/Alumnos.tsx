...
import { AlertCircle, CheckCircle, ChevronDown, Filter, Search, Trash2, User, X } from "lucide-react";
import { alumnos as initialAlumnos, type Alumno } from "../../data/alumnos";
import { membershipPlans } from "../../data/catalog";

const STATUS_OPTIONS = ["Todos", "Al dia", "Deudor"] as const;
...
export default function Alumnos() {
  ...
  const planOptions = ["Todos", ...membershipPlans.map((p) => p.name)];
  ...
  const filteredAlumnos = alumnos.filter((alumno) => {
    ...
    const matchStatus = filterStatus === "Todos" || alumno.status === filterStatus;
  ...
  const hasFilters = filterStatus !== "Todos" || filterPlan !== "Todos";
      ...
      <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none rounded-lg border border-gray-300 pl-3 pr-8 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
              >
            ...
            {hasFilters && (
              <button
                onClick={() => { setFilterStatus("Todos"); setFilterPlan("Todos"); }}
        ...
        {hasFilters && (
          <div className="mt-3 flex items-center gap-2 text-sm text-blue-700">
            ...
            Filtrando por:
            {filterStatus !== "Todos" && <span className="rounded-full bg-blue-100 px-2 py-0.5">{filterStatus}</span>}
            {filterPlan !== "Todos" && <span className="rounded-full bg-blue-100 px-2 py-0.5">{filterPlan}</span>}
          </div>
        )}
      </div>
